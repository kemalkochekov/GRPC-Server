...
func (s *ClassInfoServer) AddClass(
	ctx context.Context,
	req *classInfoProto.ClassAddRequest,
) (*classInfoProto.ClassAddResponse, error) {
...
func (s *ClassInfoServer) UpdateClass(
	ctx context.Context,
	req *classInfoProto.ClassUpdateRequest,
) (*classInfoProto.ClassUpdateResponse, error) {
	loggerGateway := logger.FromContext(ctx)
	...
	if err != nil {
		if errors.Is(err, pkgErrors.ErrNotFound) {
			logError(
				...
				span,
				err,
				"Cannot update the student in class_info due to existing references (foreign key constraint). ",
			)

			return nil, status.Errorf(
				codes.NotFound,
				"Cannot update the student in class_info due to existing references (foreign key constraint). %v",
				err,
			)
		}
...
func (s *ClassInfoServer) DeleteClassByStudent(
	ctx context.Context,
	req *classInfoProto.ClassDeleteRequest,
) (*classInfoProto.ClassDeleteResponse, error) {
	loggerGateway := logger.FromContext(ctx)
	...
	return &classInfoProto.ClassDeleteResponse{
		Message: "Successfully Deleted ClassInfo By StudentID",
	}, nil
}

func (s *ClassInfoServer) GetAllClassesByStudent(
	ctx context.Context,
	req *classInfoProto.ClassesGetRequest,
) (*classInfoProto.ClassesGetResponse, error) {
	loggerGateway := logger.FromContext(ctx)
	...
	if err != nil {
		logError(
			...
			span,
			err,
			"Failed to get all records who is attending that class from class_info:",
		)

		return nil, status.Errorf(
			codes.Internal,
			"Failed to get all records who is attending that class from class_info: %v",
			err,
		)
	}
	...
	return &classInfoProto.ClassesGetResponse{
		ClassInfo: ClassesInfo,
		Message:   "Successfully Got All ClassInfo by StudentID",
	}, nil
}
