//go:generate mockgen -source ./connection.go -destination=./mocks/connection.go -package=mock_database
package connection
...
func NewDB(ctx context.Context, cfgs configs.DatabaseConfig) (*Database, error) {
	pool, err := pgxpool.Connect(ctx, GenerateDsn(cfgs))
	if err != nil {
...
func MigrationUp(cfg configs.DatabaseConfig) error {
...
func MigrationDownAndCloseSql(cfg configs.DatabaseConfig) error {
	db, err := sql.Open("postgres", GenerateDsn(cfg))
	if err != nil {
		return err
	}
